...
namespace ScheduleSplit.CS
{
    ...
    class ScheduleCreationUtility
    {
        ...
        public void CreateAndAddSchedules(UIDocument uiDocument, FamilySymbol sheet, ViewSchedule schedule, int totalH)
        {
            Transaction tGroup = new Transaction(uiDocument.Document, "Split schedule and create sheets");
            tGroup.Start();

            // определим размер спецификации
            ScheduleHeightsOnSheet heightsOnSheet = schedule.GetScheduleHeightsOnSheet();
            double H = heightsOnSheet.ColumnHeaderHeight;
            IList<double> rowH = heightsOnSheet.GetBodyRowHeights();
            foreach (double h in rowH)
            {
                H = H + h;
            }
            // определяем число сегментов
            int split = Convert.ToInt32(H / (totalH * 0.032808) );
            // разделяем спецификацию
            schedule.Split(split);

            // размещаем сегменты спецификации на листе
            for (int i=0;i<split;i++)
            {
                //Create sheet by gotten title block type.
                ViewSheet newSheet = ViewSheet.Create(uiDocument.Document, sheet.Id);
                newSheet.Name = "Sheet" + i.ToString() + " for " + schedule.Name;
                ...
                // получить точку вставки спецификации
                if (sheet.Id != ElementId.InvalidElementId)
                {
                ...
                ScheduleSheetInstance placedInstance = ScheduleSheetInstance.Create(uiDocument.Document, newSheet.Id, schedule.Id, upperLeft, i);
        ...
        //private void AddScheduleToNewSheet(Document document, FamilySymbol sheet, ViewSchedule schedule)
        //{
        ...
        //    schedule.Split(2);
        //    ScheduleSheetInstance placedInstance = ScheduleSheetInstance.Create(document, newSheet.Id, schedule.Id, upperLeft,0);
        //    ScheduleSheetInstance placedInstance2 = ScheduleSheetInstance.Create(document, newSheet.Id, schedule.Id, upperLeft, 1);
        ...
        //    //IList<double> vs = new List<double>();
        //    //vs.Add(0.5);
        //    //vs.Add(0.5);
        //    //schedule.Split(2);
        //    //schedule.RefreshData();
            
        //    schedule.Name = "Wall My";
